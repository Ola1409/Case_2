...
#Lager nytt datasett fra df
figur <- df2 %>%
  #Filtrerer år, sysselsettingsgrupper og FoU-kostnader i alt
  filter(år >= 2015) %>%
...
df3 <- df2 %>%
  pivot_longer(5:9, names_to = "kostnad", values_to = "verdi")
...
df_longer1 <- df3 %>%
  pivot_longer(5:7, names_to = "10-99 sysselsatte", values_to = "verdi1")
...
df_longer2 <- df_longer2 %>%
  relocate("10-99 sysselsatte", .after = "kostnad") %>%
  relocate("100-499 sysselsatte", .after = "10-99 sysselsatte")

df_longer3 <- df_longer2 %>%
  pivot_longer(5:6, names_to = "10-499 sysselsatte",
               values_to = "sysselgruppe")
...
df_wide <- df_wide %>%
  pivot_longer(4:8, names_to = "sysselsatte", values_to = "sysselverdi") %>%
  rename("0-499 sysselsatte" = sysselverdi)%>%
    relocate("0-499 sysselsatte", .after = "år")
df_wide <- df_wide %>%
  pivot_longer(4:5, names_to = "sysselgruppe", values_to = "verdi")
```

```{r}
ggplot(df_wide, aes(x = verdi, fill = statistikkvariabel)) + 
  geom_histogram(binwidth = 1) +
  facet_grid(`sysselgruppe` ~ næring)
```

```{r}
#Lager plot fra nye datasettet
ggplot(figurenkel, aes(x = verdi, fill = kostnad)) + 
  geom_histogram(binwidth = 1) +
